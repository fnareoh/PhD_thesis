...
We start by defining \emph{atomic strings} which are intuitively the strings over $\Sigma$ obtained by just keeping the concatenation and splitting at union, Klenee star and parenthesis. There are $\Theta(d)$ of them. \todo{give example ?}

The basis of our approach is to essentially to store efficiently specific occurrences of prefixes of the atomic strings. We then link those occurrences we stored to test if there is a “partial” match of $R$ (Definition\ref*{def:partial_occ_regexp}). Because we store few occurrences over periodic regions of the text and their may be many ways of recomposing the partial match to consider, we formulate the problem of finding whether there is a partial match, as a graph problem then solve this problem efficiently using a circuit machinery.
% Link to sketches
