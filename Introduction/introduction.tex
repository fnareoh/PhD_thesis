...
Apart from the direct comparison of two strings, distances are also used to define approximate matching problems~\cite{landau1986efficient,landau1989fast} where for given an integer $tau$, a pattern $P$ and a text $T$, one must report all positions $j$ such that there is a substring $T[i..j]$ that is at distance at most $tau$ from $P$.
% LCS with $k$ mismatches
\todo[inline]{LCSapprox k}
% DTW
In Chapter~\ref{chap:DTW}, we consider a popular distance for temporal sequences but less common for strings: the \underline{Dynamic Time Warping (DTW) distance}~\cite{sakoe1978dynamic}. For strings, the DTW distance can be described as follows: double some character to obtain strings of equal length and in order to minimize the sum of the distances between the characters at the same positions, this sum is the DTW distance.
%
