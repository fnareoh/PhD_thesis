...
% Space seed
The ``don't care'' matching model has found a direct application in the PROSITE~\cite{hulo2006prosite} database of proteins where wildcards are supported but also more generally through space seeds~\cite{li2004patternhunter} which have been used in homology search~\cite{ma2002patternhunter}, alignment~\cite{david2011shrimp2}, assembly~\cite{birol2015spaced}, and metagenomics~\cite{bvrinda2015spaced}.
% Gapped matching
\todo[inline]{WIP Do not review: Add the gapped model and then seperate the consecutive part and then join them}
\todo[inline]{Find practical motivations for Consecutive ?}
The ``don't care'' matching model is sometimes referred to as ``gapped'' matching; however, it is not to be confused with the connected approach of searching for two patterns $P_1$ and $P_2$ that occur closly together. 
%
This problem originates from the field of document retrieval. After preprocessing a collection of strings, the \emph{two patterns document retrieval problem}~\cite{muthukrishnan2002efficient} must return all strings of the collection that contain both an occurrence of $P_1$ and $P_2$.
%Gapped matching
TODO Gapped PM
% snippet 
Kopelowitz and Krauthgamer~\cite{kopelowitz2016color} introduced the \underline{snippet} problem where we must report the occurrences of $P_1$ and $P_2$ that are closest to each other.
% consecutive
Navarro and Thankatchan~\cite{NAVARRO2016108} considered a single pattern $P$ and an interval $[\alpha,\beta]$  and searched to report all consecutive occurrences of $P$ starting at positions $(i,j)$ such that $j-i$ belongs to $[\alpha,\beta]$. Since, consecutive occurrences have been considered in many publications~\cite{DBLP:conf/fsttcs/BilleGPRS20,cpm/BilleGPS21,DBLP:journals/corr/abs-2304-00887,DBLP:journals/corr/abs-2211-16860}.
