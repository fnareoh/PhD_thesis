...
Formally, we say that a string $T$ of length $n$ is periodic with period $p$ if for all $0 \leq i < n - p$, $T[i]=T[i+p]$, and the smallest period of $T$ is simply called the period of $T$.
%
