...
\begin{frame}{Space-efficiency for regular expression}
    $O(m)$ space for the Thompson automata, can we do better ? \pause
    ...
    \begin{tabular}{l c c}
        ...
        \hline \pause
        Dictionary matching & $(P_{1}|\ldots | P_{d})$ & $\Oh(d\log m)$\\ \pause
        Wildcards matching & $P_{1}(1|\ldots|\sigma)P_{2}\ldots P_{d}(1|\ldots|\sigma)P_{d+1}$ & $\Oh(d\log m)$ \pause
    \end{tabular}
...
\begin{frame}{Summary}
    ...
    \end{myalertblock}
    \vfill
    \textbf{Not shown:} Witness mechanism, circuit machinery.
\end{frame}