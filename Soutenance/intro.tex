...
\begin{frame}{\textcolor{gray}{Sketch-based approaches to process massive \bblue{string data}}}
    ...
    %\pause
    String are used in fields such as \bgreen{Bioinformatics}, \bblue{Information Retrieval}, and \borange{Cyber-security}.\pause
...
\begin{frame}{\textcolor{gray}{Sketch-based approaches to process \bred{massive} string data}}
    ...
    {\small String are used in fields such as \bgreen{Bioinformatics}, \bblue{Information Retrieval}, and \borange{Cyber-security}.}
    \begin{center}
        \pause
        \begin{tabular}{l c c c c}
            File type & \textcolor{black!10!myblue}{Webpage} &  \textcolor{black!10!mygreen}{E. coli genome} & \textcolor{black!10!myorange}{Software repository} & \textcolor{black!10!mygreen}{Human genome}\\ \hline
...
\begin{frame}{Example of lossless sketch: Lempel--Ziv factorization}
    ...
    \begin{itemize}
        \onslide<4->{\item $T[s..(e-1)]$ occurs at least twice in $T[1..(e-1)]$,}\onslide<6->{ and}
        \onslide<6->{\item either $e=|T|$ or $T[s..e]$ occurs only once in $T[1..e]$.}
    \end{itemize}
    ...
    \onslide<9->{Used for \btheme{compression} by representing $f_i=T[s..e]$ by $(d,e-s,T[e])$.}
    \onslide<10->{In the worst case, no compression, but very efficient in practice.}
\end{frame}
