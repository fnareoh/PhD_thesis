...
\begin{enumerate}
    \item \boundaryinfo{$p_1$} and \boundaryinfo{$p_2$} for $\str{A}$;
    ...
    \begin{enumerate}
    	\item \boundaryinfo{$p_1$} of $x_A$ and $y_A$, which we refer to as \emph{secondary boundary information};
...
\begin{proposition}\label{claim:leftmost_rightmost}
...
There is a $O(g+m)$-time algorithm that computes, for each symbol $A$ of $G$, the rightmost occurrence of $p_1$ in $\str{A}$ starting before $x_A$, where $(x_A,y_A)$ is \suffixinfo{$p_2$} for $\str{A}$. 
\end{proposition}    
\begin{proof}   
Let $(x_B,y_B)$ (resp., $(x_C,y_C)$) be \suffixinfo{$p_2$} for $\str{B}$ (resp., $\str{C}$). Recall Algorithm~\ref{alg:boundary} that we used to compute boundary information. 

In Cases \ref{case:s_is_substring} and \ref{case:none_is_substring}, the \suffixinfo{$p_2$} of $A$ is the \suffixinfo{$p_2$} of $C$. Therefore, the rightmost occurrence of $p_1$ before $x_A$ in $\str{A}$ is either the last occurrence before $x_C=x_A$ in $\str{C}$, or the rightmost crossing occurrence of $p_1$ for $A$, or the rightmost occurrence of $p_1$ in $\str{B}$, and we can compute it in constant time.
...
Assume that $p_2$ is not a substring of $p_1$ and that we are given the information computed in Section~\ref{sec:boundary}. 
...
\todo[inline]{Maybe put reference to the previous section}
