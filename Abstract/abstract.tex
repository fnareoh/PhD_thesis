...
% Sketches
In this thesis we contribute multiple space- and time-efficient algorithms for various string problems, by relying on sketches: compressions (lossless or lossy) that only keep the essential characteristic of the input needed to answer a given query.%, and therefore are promising for scalability.
...
% LCS and DTW
We first study approximate matching, where we must report all occurrences at distance at most $k$ for a given similarity measure. %Motivated by the lower bounds on the computation of the most popular similarity measures, 
